...
import LlvmBoxProcess from './LlvmBoxProcess.js';
import BinaryenBoxProcess from './BinaryenBoxProcess.js';
import PythonProcess from './PythonProcess.js';
import NodeProcess from './NodeProcess.js';

import root_pack from './root_pack.js';
...
class Emception {
  ...
  async _run_process_impl(argv, opts = {}) {
    ...
    const tool_info = argv[0].endsWith('.py')
      ? 'python'
      : this.fileSystem.readFile(argv[0], { encoding: 'utf8' });
...
Comlink.expose(emception, nodeEndpoint(parentPort));
