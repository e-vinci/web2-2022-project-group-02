export default function loadPyodide(_createPyodideModule, pyodideOpts, Module = {}) {
  ...
  class ResultWrapper {
    ...
    constructor(f) {
      try {
        (this._value = f()),
          this._value instanceof ResultWrapper && (this._value = this._value.unwrap()),
          this._value instanceof Promise
            ? ((this._state = ResultWrapper._STATE.PENDING),
              (this._value = (async (v) => {
                try {
                  for (; v instanceof Promise; )
                    (v = await v) instanceof ResultWrapper && (v = v.unwrap());
                  return (this._value = v), (this._state = ResultWrapper._STATE.RESOLVED), v;
                } catch (err) {
                  throw ((this._value = err), (this._state = ResultWrapper._STATE.REJECTED), err);
                }
              })(this._value)))
            : (this._state = ResultWrapper._STATE.RESOLVED);
      } catch (err) {
        (this._state = ResultWrapper._STATE.REJECTED), (this._value = err);
      }
    }
    unwrap() {
      if (this._state === ResultWrapper._STATE.REJECTED) throw this._value;
      return this._value;
    }
    then(f) {
      const non_thenable = { unwrap: this.unwrap.bind(this) };
      return this._state === ResultWrapper._STATE.PENDING
        ? new ResultWrapper(() =>
            this._value.then(
              () => f(non_thenable),
              () => f(non_thenable),
        ...
        : new ResultWrapper(() => f(non_thenable));
    }
  }
  Module.call_asyncify = function (name, ...args) {
    return new ResultWrapper(() => {
      ...
      var runningAsync = Module.Asyncify.currData,
        prevRunningAsync = Module.Asyncify.asyncFinalizers.length > 0;
      return runningAsync && !prevRunningAsync
        ? new Promise((resolve) => Module.Asyncify.asyncFinalizers.push(resolve))
        : ret;
    });
  };
  ...
  (Module.pyproxy_alloc_map = pyproxy_alloc_map),
    ...
    (Module.disable_pyproxy_allocation_tracing = function () {
      (trace_pyproxy_alloc = function (proxy) {}), (trace_pyproxy_dealloc = function (proxy) {});
    }),
  ...
  class PyProxyBufferMethods {
    getBuffer(type) {
      let ArrayType;
      if (type && ((ArrayType = type_to_array_map.get(type)), void 0 === ArrayType))
  ...
  function toPy(obj, { depth: depth = -1 } = {}) {
    ...
    try {
      ...
      try {
        ...
        throw (e instanceof Module._PropagatePythonError && Module._pythonexc2js(), e);
      }
  ...
  function setInterruptBuffer(interrupt_buffer) {
    (Module.interrupt_buffer = interrupt_buffer), Module._set_pyodide_callback(!!interrupt_buffer);
  }
  function checkInterrupt() {
    2 === Module.interrupt_buffer[0] &&
      ((Module.interrupt_buffer[0] = 0), Module._PyErr_SetInterrupt(), Module.runPython(''));
  }
  ...
  async function loadPyodide(config) {
    ...
    setStandardStreams(
      (config = Object.assign(default_config, config)).stdin,
      config.stdout,
      config.stderr,
    ),
  ...
  (Module.fatal_error = function (e) {
    if (fatal_error_occurred)
      return (
        console.error('Recursive call to fatal_error. Inner error was:'), void console.error(e)
      );
    (fatal_error_occurred = !0),
      ...
      Module.inTestHoist ? (console.error(e.toString()), console.error(e.stack)) : console.error(e);
    try {
    ...
    (Module.runPythonSimple = function (code) {
      ...
      return Module.call_asyncify('_run_python_simple_inner', code_c_string)
        .then((res) => {
          try {
            errcode = res.unwrap();
          } catch (e) {
  ...
  return loadPyodide(pyodideOpts);
}
export { loadPyodide };
